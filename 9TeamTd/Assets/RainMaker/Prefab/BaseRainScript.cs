...
namespace DigitalRuby.RainMaker
{
    ...
    {
        [Header("Audio Settings (Inspector Only)")]
        [Range(0f, 1f)]
        public float RainVolumeInspector = 1.0f;

        [Range(0f, 1f)]
        public float WindVolumeInspector = 0.5f;
        ...
        protected virtual void Update()
        {
            ...
            if (Application.isPlaying)
            {
                if (audioSourceRainCurrent != null)
                {
                    audioSourceRainCurrent.SetTargetVolume(RainVolumeInspector);
                }
                if (audioSourceWind != null)
                {
                    audioSourceWind.SetTargetVolume(WindVolumeInspector);
                }
            }
        }
